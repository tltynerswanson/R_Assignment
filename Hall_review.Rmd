...
you could probably add more to the inspection section, and a bullet list summarizing the important details. file size, type of variables, structure of file, etc. try using the str() function.
...
```
I like that you have steps to check and make sure that the results you are getting are as expected 
...
There might be a better way to process the data into each file without writing everything else explicitly. That said, I tried using a for loop and what you did runs way faster, so maybe not! 
...
It looks like the x axis labels are overlapping on top of each other and making it so you can't see the position of each SNP. It could be that R is using the position as a character variable and so doesn't recognize position as a numeric scale. maybe try x=as.numeric(position) and see if that works better.
...
```
Nice job on this visualization, it is well organized and for the most part easy to read. I like your elegant use of pivot_longer and pivot tables. maybe try rotating the x axis labels by 90 degrees so multiple and unknown don't overlap the others. However, I'm not sure this answers the question posed. I interpreted this problem as comparing the zygosity proportions of each group (e.g. ZMMIL,ZMMLR,ZMMMR) and each sample (e.g. each Sample_ID from fang_et_al). I think you could use a similar visualization, but with the original fang_et_al data to achieve this.
...
```
Nice choice of visualization and well executed. Again, maybe try angling or shortening the x axis labels so there is no overlap.

Overall I think you did a great job. Your code is well documented and commented, and was easy to read and follow. If you plan to spend some additional time refining, I would add some more inspection steps and maybe put some more thought into the second visualization.
